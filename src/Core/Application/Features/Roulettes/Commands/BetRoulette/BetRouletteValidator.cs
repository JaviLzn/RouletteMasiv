...
namespace Application.Features.Roulettes.Commands.BetRoulette
{
    public class BetRouletteValidator : AbstractValidator<BetRouletteCommand>
    {
        ...
        private void Rules()
        {
            ...
            RuleFor(expression: p => p.Color).Empty().When(predicate: p => p.Number != null).WithMessage(errorMessage: "Color should not be sent when it is sent Number.");
            ...
            RuleFor(expression: p => p.Number).Empty().When(predicate: p => !string.IsNullOrEmpty(value: p.Color)).WithMessage(errorMessage: "Number should not be sent when it is sent Color."); ;
            ...
            RuleFor(expression: p => p).Must(predicate: p => !(p.Number == null && p.Color == null)).WithMessage(errorMessage: "You must bet on Color or Number");
        }
    }
}
